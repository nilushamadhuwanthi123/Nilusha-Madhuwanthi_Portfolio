...
(function projectFishbone() {
  ...
  const PROJECTS = [
    ...
    { key:'stockerp', emoji:'📦', title:'Stock Management ERP — Supplier & Purchase Management', tag:'Internship · EgoTechWorld', categories:['Full Stack','Internship'], status:'source',
      desc:"EgoTechWorld's 4th team project — a multi-module stock management ERP built by a 4-person team in PHP and MySQL. My part is the Supplier & Purchase Management module: the full procurement lifecycle from requisition through RFQ, purchase orders, approvals, goods receipt and quality check, to invoicing, payment and returns, plus six advanced features on top — three-way PO/GRN/invoice matching, controlled PO revision with version history, a delivery variance workbench, a budget guard, rule-based supplier scoring, and a landed cost calculator.",
      tech:['PHP','MySQL','JavaScript','Bootstrap'], repoUrl:'https://github.com/LEULEX-404/stock-management-erp',
      howItWorks:"Every procurement document moves through its own status machine enforced server-side, not just hidden in the UI. Three-way matching checks PO cost, GRN accepted quantity and invoice quantity/price against configurable tolerances before flagging an exception; a budget guard derives Committed and Spent live from each purchase line's own status rather than keeping a running total, so cancelling or partially receiving an order is correct on the very next read with nothing to recalculate by hand; and the landed cost calculator allocates shipping and customs/handling charges across accepted lines by value share, shown as an effective unit cost without ever writing back to product cost or inventory valuation.",
      problem:'Gives a real business a paper-trail-safe way to run purchasing end to end — raise a requisition, get competitive quotes, approve and place the order, receive and inspect it, match the invoice, pay it and, if something is wrong, return it — with every step logged and reversible where it needs to be.',
      whyThisWay:"Money-moving logic stays out of the client entirely: a revision to an approved order is blocked the instant any stock has been received against it, not by a UI that hides the button, but by the PO's own status machine, which is the actual guarantee that received-quantity history can never be corrupted. 14 test files and 129 real database-backed assertions back the module, run against MariaDB inside a rolled-back transaction rather than mocked." },
    { key:'hcidoctor', emoji:'🩸', title:'HCI Blood Bank App — Doctor Module', tag:'Academic · SLIIT HCI Group Project', categories:['Mobile','University'], status:'live',
      desc:'A university group project — a Flutter blood-donation/blood-bank mobile app built on Firebase. My part is the Doctor & Blood Bank module: donor coordination and comparison, blood demand and request tracking, critical-request escalation with two-person verification, a filterable request history with CSV/PDF export, and accessibility support. Now deployed as a Flutter web build on GitHub Pages, with flutter analyze clean and 225 passing tests.',
      tech:['Flutter','Dart','Firebase'], demoUrl:'https://kavindu-maduhansa.github.io/Life-Link/', repoUrl:'https://github.com/kavindu-maduhansa/Life-Link/tree/feat_Member3_DoctorM',
      ...
      whyThisWay:'Donor availability is read as three states, not two. The original code treated a missing availability field as available, which meant every donor who had ever registered showed up to blood-bank staff as ready to donate on no evidence at all. Available, unavailable and unknown are now distinct, and unknown is shown as unknown.' },
    { key:'cloudverse', emoji:'🛡️', title:'CLOUDVERSE — Cyber Security Simulation', tag:'Personal · Security', categories:['Frontend','Game','Security'], status:'live', featured:true,
      desc:'A browser-based security operations simulation: triage live security events, run forensics on an evidence board, score risk, block traffic at a cost, and correlate what you actually found into security intelligence. Built with Vite and covered by 397 passing tests across 27 test files.',
      tech:['JavaScript','Vite','Vitest','GitHub Actions'], demoUrl:'https://nilushamadhuwanthi123.github.io/cloudverse-cyber-world/', repoUrl:'https://github.com/nilushamadhuwanthi123/cloudverse-cyber-world',
      howItWorks:'A live security event feed drives an incident-response engine; findings land on a forensics evidence board where connections between items are derived from shared indicators rather than authored by hand, and the risk score and security intelligence panel are both computed from what the player has actually uncovered. CI runs lint and the full test suite on every push, and deploys to GitHub Pages on merge.',
      problem:'Makes security-operations work legible by letting you play it — triage, forensics, containment and intelligence as connected decisions instead of abstract concepts.',
      whyThisWay:'Evidence-board connections are derived from shared indicators rather than hard-coded, so the links a player finds are ones the data actually supports. Hand-authoring them would have made the forensics step a guessing game about what the designer intended rather than about the evidence.' },
    { key:'sentinel', emoji:'🛡️', title:'SENTINEL — Incident Response Platform', tag:'Personal · Security', categories:['Full Stack','Security'], status:'live', featured:true,       desc:'A defensive security operations console: a live event feed is correlated into incidents, scored for risk with the reasoning shown, mapped onto an infrastructure dependency graph, and turned into ranked containment options with their real cost. 450 tests across frontend and server, 13 reviewed pull requests, CI gating every merge.',       tech:['React','Vite','Node.js','Socket.IO','Vitest','GitHub Actions'], demoUrl:'https://nilushamadhuwanthi123.github.io/sentinel-incident-response-platform/', repoUrl:'https://github.com/nilushamadhuwanthi123/sentinel-incident-response-platform',       howItWorks:'Business rules live in pure engine modules with no React, DOM or network in them — severity, risk, correlation, topology, blast radius, attack chain, recommendations and forensics — so each one is unit-testable and re-runnable by the what-if simulator. Correlation emits exactly the signal shape the risk engine consumes, so neither re-derives the other’s judgement. The server runs a seeded, scripted simulation; with no backend configured the deployed page runs that same module in the browser rather than showing OFFLINE forever.',       problem:'Makes incident response legible: instead of a scrolling feed of individually meaningless events, an analyst sees one named incident, why those events were grouped, how far the attack has got, what it would cost if it spread, and which containment action is worth taking first.',       whyThisWay:'Every number shows its working, and every limit is stated rather than hidden. The risk score lists the signals that produced it with point values that genuinely add up. The guidance panel is deterministic rules, not a language model, and says so before it says anything else. The evidence export is called a checksum, never a signature, because anyone who can edit the record can recompute it — borrowing forensic vocabulary it could not defend would cost the project the credibility of everything else in it.' },
    { key:'precisionsnake', emoji:'🐍', title:'Precision Snake', tag:'Personal · Game', categories:['Frontend','Game'], status:'live',
      desc:'Browser game, built for fun in my free time — a polished, from-scratch Snake game in vanilla JavaScript and the HTML5 Canvas API, no frameworks or dependencies. Smooth fixed-timestep game loop, keyboard and touch controls, a persisted high score and reduced-motion support.',
      tech:['Vanilla JS','Canvas'], demoUrl:'https://nilushamadhuwanthi123.github.io/precision-snake_Game/', repoUrl:'https://github.com/nilushamadhuwanthi123/precision-snake_Game',
    ...
    { key:'flappyflight', emoji:'🐤', title:'Flappy Flight', tag:'Personal · Game', categories:['Frontend','Game'], status:'live',
      desc:'Browser game, built for fun in my free time — a Flappy Bird clone in vanilla JavaScript and the Canvas API, no frameworks or dependencies. Fixed-timestep game loop, hand-drawn bird and pipes, keyboard/touch/mouse controls, and a persisted high score.',
      tech:['Vanilla JS','Canvas'], demoUrl:'https://nilushamadhuwanthi123.github.io/flappy-flight_game/', repoUrl:'https://github.com/nilushamadhuwanthi123/flappy-flight_game',
      howItWorks:'A fixed-timestep game loop (requestAnimationFrame plus an accumulator) drives circle-vs-rectangle collision against pipes and ground, with a single flap() entry point unifying keyboard, mouse and touch input, and a high score persisted to localStorage.',
      problem:'Another frontend-fundamentals piece in the same Canvas engine pattern as Precision Snake — game-loop architecture, collision detection and input handling, without a framework.' },
    { key:'brickvector', emoji:'🧱', title:'Brick Vector', tag:'Personal · Game', categories:['Frontend','Game'], status:'live',
      desc:'Browser game, built for fun in my free time — a Breakout / brick-breaker clone in vanilla JavaScript and the Canvas API. Real angle-reflection paddle physics, five hand-designed brick layouts with escalating difficulty, and a 3-life system.',
      tech:['Vanilla JS','Canvas'], demoUrl:'https://nilushamadhuwanthi123.github.io/brick-vector_game/', repoUrl:'https://github.com/nilushamadhuwanthi123/brick-vector_game',
      howItWorks:'Ball-vs-paddle bounce angle is computed from where the ball strikes the paddle (steeper near the edges) rather than a flat bounce, and brick collision resolves only the single closest overlapping brick per physics step to avoid double-bounce or stuck-ball bugs.',
      problem:'Practice in real physics-driven collision resolution and level/difficulty progression, on top of the same fixed-timestep Canvas game-loop pattern used across these games.' },
    { key:'game2048', emoji:'🔢', title:'2048', tag:'Personal · Game', categories:['Frontend','Game'], status:'live',
      desc:'Browser game, built for fun in my free time — the classic 2048 sliding-tile puzzle in vanilla JavaScript, DOM elements and CSS Grid (no canvas this time). Merge-once-per-move logic, CSS-transition tile animation, and full keyboard/touch/accessibility support.',
      tech:['Vanilla JS','CSS Grid'], demoUrl:'https://nilushamadhuwanthi123.github.io/game-2048_game/', repoUrl:'https://github.com/nilushamadhuwanthi123/game-2048_game',
      howItWorks:'A single generic line-collapse routine slides and merges one row or column toward index 0, fed rotated coordinate views for all four directions, so merge-once-per-move correctness only has to be proven in one place.',
      problem:'A DOM/state-management variant of the same game-fundamentals practice — grid logic and animation instead of canvas rendering, plus screen-reader support via an aria-live region.' },
    { key:'precisiondrift', emoji:'🏎️', title:'Precision Drift', tag:'Personal · Game', categories:['Frontend','Game'], status:'live',
      desc:'Browser game, built for fun in my free time — a more advanced endless car-racing/traffic-dodging game in vanilla JavaScript and Canvas. Smooth lane-changing, procedurally spawned traffic, a considered difficulty curve, and coin/shield pickups.',
    ...
    { key:'taskflowapi', emoji:'⚙️', title:'TaskFlow API — DevOps & CI/CD Showcase', tag:'Personal · DevOps', categories:['Backend','DevOps'], status:'live',
    ...
    { key:'uninexcampushub', emoji:'🏫', title:'Uninex Campus Hub', tag:'Personal · Full Stack', categories:['Full Stack','Backend'], status:'live',
      desc:'A solo campus resource management and booking system, built from scratch — resource/seat setup, time-slot and full-hall booking, an admin approval workflow with QR ticketing, incident tracking, reviews and an analytics dashboard. Google sign-in, deployed live across two hosts, and built feature by feature — each one its own issue, branch and PR.',
      tech:['Spring Boot','Java','MongoDB','React','TypeScript','OAuth 2.0','Docker'],
      demoUrl:'https://nilushamadhuwanthi123.github.io/uninex-campus-hub/', repoUrl:'https://github.com/nilushamadhuwanthi123/uninex-campus-hub',
